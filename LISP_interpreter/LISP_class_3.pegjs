...
//
{
    ...
    class Function{
        ...
        call(inputs){
           ...
           $$scope.push(scope);
           console.log("arguments:", this.args.map((arg, i) => ({[arg]: inputs[i]})));
...
= "(" _ "abs"   _ expr:Expression _ ")" {
         return {call: () => Math.abs(expr.call())}
    }
...
BuiltinFunctionCall = ArithmeticFunction / ArithmeticComparison / ABS / If // ArithmeticFunction / If / Display
...
FunctionCall = "("_ name:Identifier _ args:MayBeExpressions _ ")" {
    ...
    return {call: () => {
        let func = $get_current_scope()[name];
